...
static function Init () {
	...
	for (var i : int; i < Selection.objects.Length; i++){
		...
		if (objectsSMR.Length != 0)																	// If we have objects to work with
		{
			...
			var folderName		: String	= thisObject.name;										// Get a new folder name for this object
			var fileNumber		: int		= SFB_GetFileNumber("/SFBayStudios/Exported Meshes/" + folderName, thisObject.name);		// Get a sequential number if needed
			...
			for (var o : int; o < objectsSMR.length; o++){											// For each object
				...
				var mesh					= SFB_BakeMesh(path, obj, fileNumber);					// Save the mesh
...
// Project
static function SFB_BakeMesh(path : String, obj : GameObject, fileNumber : int) : Mesh {
	...
	var filePath				= path + "/" + obj.name + "_" + fileNumber + "" + ".asset";			// File path to saved mesh
...
static function SFB_GetFolderName(parentFolder : String, value : String) : int {
...
// want to use.  If it does, it'll send back the next available name, with numbers after it.
static function SFB_GetFileNumber(parentFolder : String, fileName : String) : int {
	Debug.Log ("Checkign name for " + parentFolder + "/" + fileName);
	var x = 0;
	while (x < 5000)
	{
		var fullPath	= Application.dataPath + "" + parentFolder + "/" + fileName + "_" + x + ".prefab";
		Debug.Log("fullPath: " + fullPath);
		if (!System.IO.File.Exists(fullPath))
		{
			Debug.Log("New Number: " + x);
			return x;
		}
		else
			x++;
